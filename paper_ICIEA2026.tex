...
practice. Faults are rare events; inducing them for data
...
with abundant healthy data and minimal labeled fault data.
...
\end{equation}
where $k$ is the number of free parameters. On our trained model,
824 of 839 surviving edges select the \emph{quadratic} form; eleven
 $\sin$; two \emph{hinge}; one \emph{sigmoid}; one
$\tanh$. 
...
orthogonal signal between the two components.

\begin{table}[t]
\centering
\caption{AUC (mean\,$\pm$\,std) of the seven KAN-AE scores at $W{=}1000$ on the
...
polynomial on that compact interval (Weierstrass), making a
...
LIME--- discarding the NN at deployment. Furthermore, unlike SHAP/LIME explanations, the proposed health equations are available in closed form,
...
\href{https://doi.org/10.1109/ACCESS.2024.3430010}{doi:10.1109/ACCESS.2024.3430010}

\bibitem{britoshaprm2022}
L.~C.~Brito, G.~A.~Susto, J.~N.~Brito, and M.~A.~V.~Duarte,
``Fault detection of rotating machinery components using a stacked
autoencoder and one-class SVM with SHAP explanations,''
\emph{Mach.\ Learn.\ Appl.}, 2022.

\bibitem{cranmer2023pysr}
M.~Cranmer,
``Interpretable machine learning for science with PySR and
SymbolicRegression.jl,''
\emph{arXiv:2305.01582}, 2023.

\bibitem{cbmsurvey2025}
% TODO: verify full author list
``Recent advances in vibration condition-based fault diagnosis of
rotating machinery,''
\emph{J.\ Control Decis.}, Oct.\ 2025.
